...
export async function POST(request:Request){try{
 ...
 if(b.action==='seen'){
  ...
  await db.prepare('INSERT INTO visits(subject,seen) VALUES(?,?) ON CONFLICT(subject) DO UPDATE SET seen=MAX(seen,excluded.seen)').bind(sub,readThrough).run();const saved=(await db.prepare('SELECT seen FROM visits WHERE subject=?').bind(sub).first<{seen:number}>())?.seen||readThrough;return reply({ok:true,seen:saved});
 }
