...
export async function GET(request:Request){try{
 ...
 if(u.searchParams.has('replies')){
  ...
  const rows=replyAfterValue
   ...
   :(await db.prepare(`SELECT p.id,p.author,p.board,p.parent,p.body,p.video,p.media_type mediaType,p.media_name mediaName,p.media_size mediaSize,p.media_group mediaGroup,p.created,p.pinned,u.name,u.role,(SELECT COUNT(*) FROM likes l WHERE l.post=p.id) likes,(SELECT COUNT(*) FROM posts child WHERE child.parent=p.id AND child.status='visible') replies,EXISTS(SELECT 1 FROM likes l WHERE l.post=p.id AND l.user=?) liked FROM posts p JOIN users u ON u.id=p.author WHERE p.parent IN (${replyMarks}) AND p.status='visible' ORDER BY p.created ASC,p.id ASC LIMIT 21`).bind(me?.id||'',...replyParents).all()).results;
  const replyPage=rows.slice(0,20) as Record<string,unknown>[];const replyLast=replyPage[replyPage.length-1] as {created:number;id:string}|undefined;const replyMore=rows.length>20;
  return reply({posts:await enrichPosts(replyPage,me?.id||''),nextAfter:replyMore&&replyLast?afterFor(replyLast):null});
 }
 ...
 const previousSeen=(await db.prepare('SELECT seen FROM visits WHERE subject=?').bind(readSubject).first<{seen:number}>())?.seen||0;
 const unread=board&&previousSeen?Number((await db.prepare(`SELECT COALESCE(SUM(CASE WHEN ${logicalPostAnchor} THEN 1 ELSE 0 END),0) count FROM posts p WHERE p.board=? AND p.status='visible' AND p.created>? AND NOT EXISTS(SELECT 1 FROM users own WHERE own.id=p.author AND own.subject=?) AND (p.parent IS NULL OR EXISTS(SELECT 1 FROM posts parent WHERE parent.id=p.parent AND parent.status='visible'))`).bind(board,previousSeen,readSubject).first<{count:number}>())?.count||0):0;
 stats.unread=unread;
 ...
 return reply({me:publicMe,anonymous:session.anonymous,month:requested,currentMonth:current,availableMonths,boards,board,posts:await withMediaItems(pageRows,me?.id||''),nextCursor,poll,mine,video:publicVideo,mediaGroup:requestedGroup||null,mediaItems,stats,previousSeen,viewUntil,flags});
...
export async function POST(request:Request){try{
 ...
 if(b.action==='seen'){
  if(!Number.isSafeInteger(b.until)||Number(b.until)<0||Number(b.until)>now)throw new Error('invalid_request');
  const seenMonth=monthJST();const seenBoards=confirmedCharactersForMonth(seenMonth).map(character=>`${seenMonth}:${character.id}`);let readThrough=1;
  if(seenBoards.length){const marks=seenBoards.map(()=>'?').join(',');const latest=await db.prepare(`SELECT MAX(p.created) seen FROM posts p WHERE p.status='visible' AND p.board IN (${marks}) AND (p.parent IS NULL OR EXISTS(SELECT 1 FROM posts parent WHERE parent.id=p.parent AND parent.status='visible'))`).bind(...seenBoards).first<{seen:number}>();readThrough=Math.max(1,Number(latest?.seen||0));}
  await db.prepare('INSERT INTO visits(subject,seen) VALUES(?,?) ON CONFLICT(subject) DO UPDATE SET seen=MAX(seen,excluded.seen)').bind(seenSubject,readThrough).run();const saved=(await db.prepare('SELECT seen FROM visits WHERE subject=?').bind(seenSubject).first<{seen:number}>())?.seen||readThrough;return reply({ok:true,seen:saved});
 }
