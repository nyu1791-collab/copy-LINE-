...
export async function POST(request:Request){try{
 const h=await headers();const u=new URL(request.url);const origin=h.get('origin');if(!origin||origin!==u.origin||h.get('sec-fetch-site')==='cross-site')throw new Error('forbidden');
