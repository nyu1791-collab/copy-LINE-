...
export async function POST(request:Request){try{
 const h=await headers();const origin=h.get('origin');const network=await abuseNetworkBucket(h);
 // Telemetry is best-effort. Do not accept a client-asserted subject as an
 // identity or use it as an unbounded D1 key; a real edge request has a
 // privacy-preserving network bucket, while unsupported callers are ignored.
 if(!network||!origin||origin!==new URL(request.url).origin||h.get('sec-fetch-site')==='cross-site')return response(204);
 ...
 const now=Date.now();const admitted=await database().prepare('INSERT INTO limits(key,count,until) VALUES(?,1,?) ON CONFLICT(key) DO UPDATE SET count=CASE WHEN until<=? THEN 1 ELSE count+1 END, until=CASE WHEN until<=? THEN excluded.until ELSE until END WHERE until<=? OR count<? RETURNING count').bind('telemetry:'+network,now+60000,now,now,now,30).first();
