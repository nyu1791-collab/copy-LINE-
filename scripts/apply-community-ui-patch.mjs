...
if(!tests.includes(marker)){
 tests+=`\n\ntest('media composer supports ten images and five videos per post', async () => {\n  const source = await readFile(new URL('../app/community.tsx', import.meta.url), 'utf8');\n  const rules = await readFile(new URL('../lib/rules.ts', import.meta.url), 'utf8');\n  const directUpload = await readFile(new URL('../app/api/upload/route.ts', import.meta.url), 'utf8');\n  const videoSession = await readFile(new URL('../app/api/upload/session/route.ts', import.meta.url), 'utf8');\n  assert.match(rules, /maxImagesPerPost=10/);\n  assert.match(rules, /maxVideosPerPost=5/);\n  assert.match(source, /1回の投稿につき動画は最大5本・画像は最大10枚/);\n  assert.match(source, /videoCount>maxVideosPerPost/);\n  assert.match(source, /imageCount>maxImagesPerPost/);\n  assert.match(source, /else void uploadImage\(fileToSend,description,request,uploadBoard,group\)/);\n  assert.match(directUpload, /groupLimit=video\?maxVideosPerPost:maxImagesPerPost/);\n  assert.match(videoSession, /maxVideosPerPost/);\n  assert.match(videoSession, /status='uploading' AND media_type LIKE 'video\/%'/);\n});\n`;
