...
export function auditCommunityRelease(snapshot,registry,state){
 ...
 for(const topic of topics){
  ...
  if(topic?.source==='pvp-auto'){
   ...
   if(!evidence||evidence.catalogId!==id||evidence.releaseMonth!==topicMonth||evidence.source!=='notice2.line.me/LGRGS/ios/document/notice'||!Number.isSafeInteger(evidence.noticeId)||evidence.noticeId<1||typeof evidence.noticeTitle!=='string'||!/\bnew rangers? are here!?(?=\W|$)/i.test(evidence.noticeTitle)||typeof evidence.matchedName!=='string'||evidence.matchedName!==topic.nameEn||!Number.isSafeInteger(evidence.grade)||evidence.grade!==topic.verifiedGrade||!Number.isFinite(Date.parse(evidence.publishedAt))||evidence.noticeUrl!=='https://notice2.line.me/LGRGS/ios/document/notice')communityErrors.push('missing or invalid official release evidence for '+id);
