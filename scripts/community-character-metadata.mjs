...
function officialSkillDetails(ranger,skills,catalogs){
 ...
 for(const code of codes){
  ...
  const chance=Number(skill.probability),cooldown=Number(skill.skillDelayTime),area=Number(skill.range);
  if(skill.probability==null||skill.skillDelayTime==null||skill.range==null||!Number.isFinite(chance)||chance<0||chance>1||Math.abs(chance*100-Math.round(chance*100))>0.001||!Number.isSafeInteger(cooldown)||cooldown<0||cooldown>180||!Number.isSafeInteger(area)||area<1||area>5000)return null;
  ...
  for(const language of ['ja','en','zh']){
   ...
   if(!title||!description||description.length>1200||/[\u0000-\u0008\u000b\u000c\u000e-\u001f\u007f<>]/u.test(description)||!/(?:^|\n)\s*[*＊•・]\s*\S/u.test(description.replace(/\\n/g,'\n')))return null;
  }
 }
...
export async function scanOfficialRangerReleaseNotices(catalogEntries,{fetchImpl=fetch,now=Date.now(),maxPages=8}={}){
 ...
 for(const document of documents){
  ...
  const maintenanceDates=[...detail.body.matchAll(/(?:(\d{1,2})[./-](\d{1,2})(?:[./-](20\d{2}))?\s+maintenance|maintenance\s+(?:on|of)\s+(\d{1,2})[./-](\d{1,2})(?:[./-](20\d{2}))?)/gi)].map(match=>({month:Number(match[1]||match[4]),day:Number(match[2]||match[5]),year:match[3]||match[6]?Number(match[3]||match[6]):null}));
  ...
  const bodyEnd=maintenanceDates[1];
  if(start.month<1||start.month>12||start.day<1||start.day>31||end.month<1||end.month>12||end.day<1||end.day>31||bodyEnd.month!==end.month||bodyEnd.day!==end.day||bodyEnd.year&&end.year&&bodyEnd.year!==end.year)continue;
...
export function createOfficialCharacterVerifier(fetchImpl=fetch){
 ...
 verify.findOfficialReleaseEvidence=async(now=Date.now())=>{
  ...
  return scanOfficialRangerReleaseNotices(entries,{fetchImpl,now});
 };
 return verify;
}
