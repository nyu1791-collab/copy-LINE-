...
function validateOfficialMetadata(value,id){
 ...
 if(!name||!nameEn||!nameZh||typeof value.unitNameCode!=='string'||!/^[A-Za-z0-9_-]{1,120}$/.test(value.unitNameCode))return null;
 return {id,name,nameEn,nameZh,nameTh,unitNameCode:value.unitNameCode,stage:'e',grade:Number.isSafeInteger(value.grade)?value.grade:null,source:'rangers.lerico.net/api/getRangersBasics',verifiedAt:typeof value.verifiedAt==='string'?value.verifiedAt:null};
}
...
export async function updateCommunityCharacters({snapshot,history,registry,state,legacyKnown,probe=probeImage,verifyMetadata}={}){
 ...
 for(const row of rows){
  ...
  const image=safeImage(row.image,id);
  const baseEligible=SAFE_ID.test(id)&&!!image;
  ...
  const record={id,name,image,metadata,metadataVerified:!!metadata,verifiedImageUrl:imageVerified?image:null,firstSeenAt,firstSeenMonth,lastSeenAt:updatedAt,consecutive,eligible,imageVerified,pvpRank:snapshotRank(row),adoptionRate:adoptionRate(row)};
  nextState.candidates[id]=record;
  if(eligible&&imageVerified&&consecutive>=REQUIRED_CONSECUTIVE&&firstSeenMonth===releaseMonth){
   const topic={id,name:metadata.name,nameEn:metadata.nameEn,nameZh:metadata.nameZh,...(metadata.nameTh?{nameTh:metadata.nameTh}:{}),image,releaseMonth,confirmed:true,source:'pvp-auto',metadataSource:metadata.source,unitNameCode:metadata.unitNameCode,evolutionStage:metadata.stage,verifiedGrade:metadata.grade,observationCount:consecutive,firstObservedAt:firstSeenAt,confirmedAt:updatedAt,pvpRank:snapshotRank(row),adoptionRate:adoptionRate(row)};
