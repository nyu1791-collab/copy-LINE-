...
test('next month promotion appends a board without changing the old board identity or stored discussion',async()=>{
 ...
 assert.match(boardApi,/SELECT DISTINCT month FROM boards/);
 assert.doesNotMatch(boardApi,/DELETE FROM boards|DROP TABLE boards/);
});

test('September stays closed to new automatic boards even when a matching notice exists',async()=>{
