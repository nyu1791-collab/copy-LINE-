...
test('comment UI keeps reactions private, opens replies on demand, and marks a selected reply target',()=>{
 ...
 assert.match(communitySource,/mine/);assert.match(communitySource,/composer-reply/);assert.match(communitySource,/t\.replyTargetLabel/);assert.match(communitySource,/権限・バッジ管理（Owner・モデレーター）/);assert.match(communitySource,/動画制作貢献者/);assert.match(communitySource,/有益情報貢献者/);assert.match(communitySource,/ownerDisplayName/);assert.doesNotMatch(communitySource,/運営バッジ/);assert.match(communitySource,/data\?\.me\?\.role==='owner'/);
