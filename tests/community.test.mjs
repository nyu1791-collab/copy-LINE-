...
test('owner activation exchanges a private access key for a signed cookie without login',async()=>{
 ...
 assert.match(activated.setCookie,/Max-Age=31536000/);const session=await anonymous.sessionFromHeaders(new Headers({cookie:activated.setCookie}));assert.equal(session.owner,true);assert.equal(session.sub,'owner-subject');assert.match(session.setCookie||'',/^__Host-lr_owner=o1\.[0-9]+\./);
 ...
 const displayCookie=(savedCookie.match(/__Host-lr_display_name=[^;]+/)||[''])[0];const restoredSession=await anonymous.sessionFromHeaders(new Headers({cookie:`${activated.setCookie}; ${displayCookie}`}));assert.equal(restoredSession.owner,true);assert.equal(restoredSession.displayName,'保存した運営名');
