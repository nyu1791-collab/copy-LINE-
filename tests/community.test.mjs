...
test('comment UI keeps reactions private, opens replies on demand, and marks a selected reply target',()=>{
 ...
 assert.match(communitySource,/replyTarget/);assert.match(communitySource,/toggleReplies/);assert.match(communitySource,/t\.showReplies\.replace/);assert.doesNotMatch(communitySource,/translatePost|<Languages/);assert.match(communitySource,/replyingTo/);assert.match(communitySource,/post-meta.*badges\.map/s);assert.match(communitySource,/data-badge=\{badge\}/);assert.match(communitySource,/post-author-line/);assert.match(communitySource,/post-badges/);assert.match(communitySource,/data-badge=\{badge\}/);assert.match(communityCss,/\.post-badges[^}]*margin-left:auto/);
