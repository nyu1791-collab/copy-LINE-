...
test("Workers runtime keeps the PvP shell free of the obsolete community entry and supports anonymous browsing", async () => {
  const mf = new Miniflare({
    ...
    bindings: {BOARD_ANON_COOKIE_SECRET:"test-anon-cookie-secret-0123456789012345",BOARD_TRUST_UPSTREAM_AUTH:"test"},
    d1Databases: ["DB"],
    r2Buckets: ["BUCKET"],
  });
  try {
    ...
    const uploadRequest=new Request('https://review.example/api/upload',{method:'PUT',headers:{cookie:guestCookie,host:'review.example',origin:'https://review.example'},body:form});const upload=await mf.dispatchFetch(uploadRequest.url,{method:'PUT',headers:Object.fromEntries(uploadRequest.headers),body:new Uint8Array(await uploadRequest.arrayBuffer())});const uploaded=await upload.json();assert.equal(upload.status,200,JSON.stringify(uploaded));
    ...
    const boundedOpenEnded=await mf.dispatchFetch('https://review.example/api/media?id='+completeResult.id,{headers:{'oai-authenticated-user-id':'isolated-worker-test',range:'bytes=0-'}});assert.equal(boundedOpenEnded.status,206);const explicitRangeBytes=Math.min(video.length,16*1024*1024);assert.equal(boundedOpenEnded.headers.get('content-range'),`bytes 0-${explicitRangeBytes-1}/${video.length}`);assert.equal((await boundedOpenEnded.arrayBuffer()).byteLength,explicitRangeBytes);
